...
namespace Microsoft.AspNetCore.Blazor.Server.Circuits
{
    internal class CircuitHost : IDisposable
    {
        ...
        public async void BeginInvokeDotNetFromJS(string callId, string assemblyNameOrDotNetObjectId, string methodIdentifier, string argsJson)
        {
            AssertInitialized();
            ...
            try
            {
                await SynchronizationContext.Invoke(() =>
                {
                    ...
                    DotNetDispatcher.BeginInvoke(callId, assemblyNameOrDotNetObjectId, methodIdentifier, argsJson);
                });
            }
            catch (Exception ex)
            {
